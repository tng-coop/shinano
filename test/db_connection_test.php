...
$dbconn_ro = new PDO($dsn, $config['database']['readonly_user'], $config['database']['readonly_password']); 
$stmt_ro = $dbconn_ro->query("SELECT 1");
$row = $stmt_ro->fetch(PDO::FETCH_ASSOC);
print_r($row);

// Connecting to the database using read-write credentials
$dbconn_rw = new PDO($dsn, $config['database']['readwrite_user'], $config['database']['readwrite_password']);
